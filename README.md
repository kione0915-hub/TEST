...
**🔔 알림 조건**: 대시보드에서 원하는 순간에만 알림을 받도록 고를 수 있습니다.
- MACD 골든/데드크로스, 볼린저 하단 이탈/상단 돌파, RSI 과매도/과매수 — 각각 켜고 끄기
- **지표 설정**: MACD(단기/장기/시그널), 볼린저(기간/표준편차 배수), RSI(기간/기준선)를
  원하는 값으로 변경 가능 — 저장 즉시 알림 판정과 차트에 반영
- 종목별 목표가 알림 (지정 가격 이상/이하 도달 시)
- 같은 조건이 유지되는 동안은 다시 알리지 않고, 새로 발생한 순간에만 알립니다
- 꺼둔 조건은 알림뿐 아니라 자동주문 판단에서도 제외됩니다
- 자동매매 실행 중에는 지표 값이 대시보드에 실시간(주기마다) 갱신됩니다

**📊 차트**: 종목 버튼을 누르면 최근 60일 차트가 3단으로 표시됩니다.
- 종가 + 볼린저 밴드 / MACD(선·시그널·히스토그램) / RSI(과매수·과매도 기준선)
- 마우스를 올리면 날짜별 상세 값이 툴팁으로 표시됩니다
